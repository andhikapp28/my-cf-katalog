...
export default async function AdminCirclesPage({
  ...
  return (
    <AdminCreateToggle>
      <AdminShell
        title="Circles"
        description="Kelola directory circle global, link sosial utama, dan catatan yang nanti dipakai ulang lintas event."
        headerActions={<AdminCreateToggleButton label="Add circle" />}
      >
          ...
          {pagination.totalItems ? (
            <>
              ...
              <AdminDataTable className="hidden md:block" tableClassName="w-full table-fixed">
                <thead>
                  <tr>
                    <AdminTableHead className="w-[34%] px-3">Circle</AdminTableHead>
                    <AdminTableHead className="w-[26%] px-3">Social</AdminTableHead>
                    <AdminTableHead className="w-[30%] px-3">Notes</AdminTableHead>
                    ...
                    return [
                      <tr key={circle.id} className="bg-white/80 hover:bg-brand-50/30">
                        <AdminTableCell className="px-3 py-3 align-top">
                          ...
                          <p className="mt-1 truncate text-sm text-ink-500">{circle.slug}</p>
                        </AdminTableCell>
                        <AdminTableCell className="px-3 py-3 align-top">
                          {circle.socialLink ? (
                            <a href={circle.socialLink} target="_blank" rel="noreferrer" className="block overflow-hidden truncate font-medium text-brand-700">
                              {compactUrl(circle.socialLink, 22)}
                            </a>
                          ) : (
