      ...
      >
        ...
        <section className="space-y-4">
          <AdminSectionHeading
            ...
            actions={
              <>
                <AdminCreateToggleButton label="Add expense" />
                <Button asChild variant="secondary">
          ...
          {pagination.totalItems ? (
            <>
                  ...
                  <article key={expense.id} className="panel p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-[var(--font-display)] text-xl font-semibold text-ink-900">{expense.product?.name ?? "General expense"}</p>
              ...
              <AdminDataTable className="hidden lg:block" tableClassName="w-full table-fixed">
                <thead>
                  <tr>
                    <AdminTableHead className="w-[96px] px-3">Date</AdminTableHead>
                    <AdminTableHead className="w-[22%] px-3">Event</AdminTableHead>
                    <AdminTableHead className="w-[18%] px-3">Category</AdminTableHead>
                    <AdminTableHead className="w-[30%] px-3">Expense</AdminTableHead>
                    <AdminTableHead className="w-[96px] px-3">Amount</AdminTableHead>
                    <AdminTableHead className="w-[100px] px-3">State</AdminTableHead>
                    <AdminTableHead className="w-[72px] px-3 text-right">Actions</AdminTableHead>
                  </tr>
                </thead>
                <tbody>
                  {pagination.items.map((expense) => (
                    <tr key={expense.id} className="bg-white/80 align-top hover:bg-brand-50/30">
                      <AdminTableCell className="px-3 py-3 align-top text-ink-600">
                        <span className="block font-medium">{formatDate(expense.expenseDate)}</span>
                      </AdminTableCell>
                      <AdminTableCell className="px-3 py-3 align-top">
                        <p className="truncate font-medium text-ink-900">{expense.event.name}</p>
                        <p className="mt-1 truncate text-sm text-ink-500">{formatEnumLabel(expense.paymentMethod)}</p>
                      </AdminTableCell>
                      <AdminTableCell className="px-3 py-3 align-top">
                      ...
                      <AdminTableCell className="px-3 py-3 align-top">
                        <p className="truncate font-medium text-ink-900">{expense.product?.name ?? "General expense"}</p>
                        <p className="mt-1 text-sm text-ink-600" style={notesClampStyle}>
                          {expense.note ? truncateText(expense.note, 72) : "No note"}
                        </p>
                      </AdminTableCell>
                      <AdminTableCell className="px-3 py-3 align-top font-medium text-ink-900">
                      ...
                      <AdminTableCell className="px-3 py-3 text-right align-middle">
                        <AdminActionDropdown>
                          <AdminActionLink href={buildPathWithQuery("/admin/expenses", { ...sharedQuery, edit: expense.id })}>
                            Edit
                          </AdminActionLink>
