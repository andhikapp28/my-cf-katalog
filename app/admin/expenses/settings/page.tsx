  ...
  return (
    <AdminCreateToggle>
      <AdminShell
        ...
        headerActions={<AdminCreateToggleButton label="Add category" />}
      >
        ...
        <section className="space-y-4">
          ...
          {categories.length ? (
            <AdminDataTable tableClassName="w-full table-fixed">
              <thead>
                <tr>
                  <AdminTableHead className="w-[28%]">Name</AdminTableHead>
                  <AdminTableHead className="w-[24%]">Slug</AdminTableHead>
                  <AdminTableHead className="w-[24%]">Color</AdminTableHead>
                  <AdminTableHead className="w-[180px] text-right">Actions</AdminTableHead>
                </tr>
              </thead>
              <tbody>
                {categories.map((category) => {
                  ...
                  return [
                    <tr key={category.id} className="bg-white/80 hover:bg-brand-50/40">
                      <AdminTableCell className="font-medium text-ink-900">
                        <span className="block truncate">{category.name}</span>
                      </AdminTableCell>
                      <AdminTableCell className="text-ink-500">
                        <span className="block truncate">{category.slug}</span>
                      </AdminTableCell>
                      <AdminTableCell>
                        <div className="flex items-center gap-3">
                          <span className="h-8 w-16 rounded-xl border border-line" style={{ backgroundColor: category.color }} />
                          <span className="truncate text-sm text-ink-500">{category.color}</span>
                        </div>
                      </AdminTableCell>
